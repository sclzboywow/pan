...
class ProxyDownloadWorker(QThread):
    ...
    def __init__(self, base_url: str, ticket: str, save_path: str, tmp_path: str, size_expect: int = 0, app_jwt: str = None, resume_pos: int = 0, mode_token: int = None, parent=None):
        super().__init__(parent)
        ...
        self.mode_token = mode_token  # 模式版本号，用于验证
        self._stopped = False
...
class FileManagerUI(QMainWindow):
    ...
    def set_mode(self, mode: str) -> bool:
        """统一管理模式切换，避免状态残留
        参数: mode - "public" 或 "user"
        返回: True 表示切换成功，False 表示切换失败
        """
        try:
            if mode not in ["public", "user"]:
                print(f"[ERROR] 无效的模式: {mode}")
                return False
            
            if self.current_mode == mode:
                print(f"[DEBUG] 已在目标模式: {mode}")
                return True
            
            print(f"[DEBUG] 切换模式: {self.current_mode} -> {mode}")
            ...
            if mode == "public":
                ...
                self._init_user_mode()
            
            print(f"[DEBUG] 模式切换完成: {old_mode} -> {mode}, token={self.mode_token}")
            return True
    ...
    def _init_public_mode(self):
        ...
        try:
            ...
            print(f"[ERROR] 初始化公共模式失败: {e}")
    
    def _init_user_mode(self):
        """初始化用户模式"""
        try:
            self.file_tree.show()
            self.icon_grid.hide()
            self.status_label.setText("用户态：加载中...")
        except Exception as e:
            print(f"[ERROR] 初始化用户模式失败: {e}")
    
    def _validate_payload_mode(self, payload, expected_mode):
        """验证列表项数据的模式标记和版本号
        参数: payload - 列表项数据, expected_mode - 期望的模式
        返回: True 表示验证通过，False 表示验证失败
        """
        try:
            if not isinstance(payload, dict):
                print(f"[WARNING] 无效的payload数据: {payload}")
                return False
            
            # 检查模式标记
            payload_mode = payload.get("mode")
            if payload_mode != expected_mode:
                print(f"[WARNING] 模式不匹配: 期望={expected_mode}, 实际={payload_mode}")
                return False
            
            # 检查版本号（可选，用于防止跨模式执行）
            payload_token = payload.get("token")
            if payload_token is not None and payload_token != self.mode_token:
                print(f"[WARNING] 模式版本号不匹配: 期望={self.mode_token}, 实际={payload_token}")
                return False
            
            return True
            
        except Exception as e:
            print(f"[ERROR] 验证payload模式失败: {e}")
            return False
    
    def get_user_row_payload(self, row):
        """获取用户态表格行的数据
        参数: row - 行号
        返回: dict 包含 {'file': raw, 'fsid': ..., 'path': ...} 或 None
        """
        try:
            ...
            payload = name_item.data(Qt.UserRole)
            if not isinstance(payload, dict):
                return None
            
            # 验证模式
            if not self._validate_payload_mode(payload, "user"):
                return None
            
            return {
                'file': payload.get('raw', {}),
            ...
            print(f"[ERROR] 获取行数据失败: {e}")
            return None

    def _ensure_mode(self, require_public: bool, scene: str = "操作") -> str:
        """确保当前处于期望态；不满足时提示并可一键切换。
        返回: "ok" 表示已在期望态；"switching" 表示已触发切换；"canceled" 表示用户取消。
        """
        try:
            target_mode = "public" if require_public else "user"
            
            if self.current_mode == target_mode:
                return "ok"
            
            # 模式不匹配，提示用户切换
            mode_name = "公共资源" if require_public else "我的网盘"
            current_mode_name = "公共资源" if self.in_public else "我的网盘"
            
            reply = QMessageBox.question(
                self,
                "模式不匹配",
                f"当前在'{current_mode_name}'，需在'{mode_name}'完成{scene}。是否切换到{mode_name}？",
            ...
            print(f"[ERROR] _ensure_mode 异常: {e}")
            return "canceled"
    ...
    def load_files(self):
        ...
        # 确保在用户态
        if not self.set_mode("user"):
            print("[ERROR] 切换到用户态失败")
            return
    ...
    def display_user_files(self, files, append: bool = False):
        ...
        try:
            ...
            for f in files:
                ...
                # 统一的数据结构，包含模式标记和完整上下文
                payload = {
                    "mode": "user",
                    "fsid": fs_id,
                    "path": path_val,
                    "raw": f,
                    "token": self.mode_token  # 当前模式版本号
                }
                name_item.setData(payload, Qt.UserRole)
    ...
    def on_user_cell_clicked(self, index):
        ...
        try:
            ...
            if mode_result != "ok":
                if mode_result == "switching":
                    self.status_label.setText("已切换到我的网盘，请重新操作")
                return
    ...
    def share_user_file(self, file_info: dict):
        ...
        try:
            fsid = file_info.get('fs_id') or file_info.get('fsid')
            if not fsid:
                QMessageBox.warning(self, "分享", "无法获取文件ID")
                return
    ...
    # ---------- 通用异步更新操作（新建/重命名/移动/复制） ----------
    def _start_op_worker(self, op_name: str, args: dict, verify: dict):
        """启动通用异步操作线程并处理UI反馈"""
        try:
            # 并发保护：避免与已有操作冲突
            if hasattr(self, 'op_worker') and self.op_worker and self.op_worker.isRunning():
                QMessageBox.warning(self, "操作进行中", "请等待当前操作完成后再试")
                return

            from ui.threads.op_worker import OperationWorker
            print(f"[DEBUG][UI] start op_worker op={op_name} args={args} verify={verify}")
            self.op_worker = OperationWorker(self.api_client, op_name, args, verify, self)
            self.op_worker.op_started.connect(self._on_op_started)
            self.op_worker.op_progress.connect(self._on_op_progress)
            self.op_worker.op_completed.connect(self._on_op_completed)
            self.op_worker.start()
        except Exception as e:
            QMessageBox.critical(self, "操作异常", str(e))

    def _on_op_started(self, op_name: str):
        print(f"[DEBUG][UI] op_started {op_name}")
        if hasattr(self, 'statusBar') and self.statusBar:
            self.statusBar.showMessage(f"正在{self._op_cn(op_name)}...")

    def _on_op_progress(self, op_name: str, message: str):
        print(f"[DEBUG][UI] op_progress {op_name}: {message}")
        if hasattr(self, 'statusBar') and self.statusBar:
            self.statusBar.showMessage(f"{self._op_cn(op_name)}进度: {message}")

    def _on_op_completed(self, op_name: str, success: bool, message: str):
        print(f"[DEBUG][UI] op_completed {op_name} success={success} msg={message}")
        try:
            # 清理线程
            if hasattr(self, 'op_worker') and self.op_worker:
                if self.op_worker.isRunning():
                    self.op_worker.quit()
                    self.op_worker.wait(3000)
                self.op_worker.deleteLater()
                delattr(self, 'op_worker')
            ...
            # 结果提示
            if success:
                QMessageBox.information(self, self._op_cn(op_name), message or "操作成功")
            else:
                msg = message or ""
                try:
                    text = str(msg)
                    ...
                    text = ""
                if "已提交" in text:
                    QMessageBox.information(self, self._op_cn(op_name), text)
                else:
                    QMessageBox.warning(self, f"{self._op_cn(op_name)}失败", text or "操作失败")
        finally:
            if hasattr(self, 'statusBar') and self.statusBar:
                self.statusBar.clearMessage()

    def _op_cn(self, op_name: str) -> str:
        mapping = {
            'mkdir': '新建文件夹',
            'rename': '重命名',
            'move': '移动',
    ...
    def open_public_resources(self):
        """打开公共资源页（在主内容区加载）"""
        # 切换到公共态
        if not self.set_mode("public"):
            print("[ERROR] 切换到公共态失败")
            return
    ...
    def on_public_cell_clicked(self, index):
        ...
        try:
            ...
            if mode_result != "ok":
                if mode_result == "switching":
                    self.status_label.setText("已切换到公共资源，请重新操作")
                return
            if not index.isValid():
                return
            row = index.row()
            ...
            # 验证模式标记和版本号
            if not self._validate_payload_mode(payload, "public"):
                return
                
            fs_id = payload.get('fsid')
    ...
    def show_context_menu(self, position):
        ...
        try:
            ...
            # 用户态模式
            # 态势校验：必须在用户态
            mode_result = self._ensure_mode(False, "右键菜单")
            if mode_result != "ok":
                if mode_result == "switching":
                    self.status_label.setText("已切换到我的网盘，请重新操作")
                return
            ...
            # 登录校验
            if not self.api_client.is_logged_in():
                QMessageBox.information(self, "右键菜单", "请先登录后再使用此功能")
                return
            ...
            if index.isValid():
                ...
                if action == act_new_folder:
                    ...
                    if ok and text.strip():
                        import posixpath
                        name = text.strip()
                        cur_dir = self.current_folder or '/'
                        self._start_op_worker(
                            'mkdir',
                            {'dir_path': cur_dir, 'folder_name': name},
                            {'current_dir': cur_dir, 'expect_path': posixpath.join(cur_dir, name)}
                        )
                    return
                if action == act_rename:
                    ...
                    if ok and new_name.strip():
                        import posixpath
                        file_path = row_data.get('path') or '/'
                        dir_path = posixpath.dirname(file_path)
                        self._start_op_worker(
                            'rename',
                            {'file_path': file_path, 'new_name': new_name.strip()},
                            {'current_dir': dir_path, 'expect_path': posixpath.join(dir_path, new_name.strip())}
                        )
                    return
                if action == act_move:
                    if not fs_id:
                    ...
                    if dialog.exec() == QDialog.Accepted:
                        ...
                        if target_path != self.current_folder:
                            import posixpath
                            file_path = row_data.get('path') or '/'
                            base_name = posixpath.basename(file_path.rstrip('/'))
                            self._start_op_worker(
                                'move',
                                {'source_path': file_path, 'target_dir': target_path},
                                {'current_dir': target_path, 'expect_path': posixpath.join(target_path, base_name)}
                            )
                    return
                if action == act_copy:
                    if not fs_id:
                ...
                if action == act_upload_local:
                    ...
                    if file_path:
                        ...
                        cur_dir = self.current_folder or '/'
                        # 仅允许上传到 /用户上传
                        if not cur_dir.startswith('/用户上传'):
                            QMessageBox.warning(self, "上传", "用户态上传路径必须在 /用户上传 下")
                            return
                        remote_path = posixpath.join(cur_dir, filename)
                        self.status_label.setText("正在上传文件...")
                        resp = self.api_client.user_upload_local_file(file_path, remote_path)
                    ...
                    if ok and text.strip():
                        ...
                        if ok2 and name.strip():
                            ...
                            if not cur_dir.startswith('/用户上传'):
                                QMessageBox.warning(self, "上传", "用户态上传路径必须在 /用户上传 下")
                                return
                            # 按API规范传 dir + filename
                            dir_path = cur_dir
                            filename = name.strip()
                            self.status_label.setText("正在上传文本...")
                            resp = self.api_client.user_upload_text(text, dir_path, filename)
            ...
            if action == act_new_folder:
                ...
                return
            if action == act_rename:
                if not (row_data.get('path')):
                    QMessageBox.warning(self, "重命名", "无法获取路径")
                    return
                new_name, ok = QInputDialog.getText(self, "重命名", "新名称：")
                if ok and new_name.strip():
                    import posixpath
            ...
            if action == act_move:
                if not (row_data.get('path')):
                    QMessageBox.warning(self, "移动", "无法获取路径")
                    return
                ...
                if dialog.exec() == QDialog.Accepted:
                    import posixpath
                    target = dialog.path_input.text().strip() or dialog.current_folder or '/'
                    target = posixpath.normpath(target) if target else '/'
                    if target:
                        file_path = row_data.get('path') or '/'
                        base_name = posixpath.basename(posixpath.normpath(file_path).rstrip('/'))
                        expect_path = posixpath.normpath(posixpath.join(target, base_name))
                        self._start_op_worker(
                            'move',
                            {'source_path': file_path, 'target_dir': target},
                            {'current_dir': target, 'expect_path': expect_path}
                        )
                return
            if action == act_copy:
                if not (row_data.get('path')):
                    QMessageBox.warning(self, "复制", "无法获取路径")
                    return
                ...
                if path:
                    ...
                    try:
                        ...
                        if file_size > 100 * 1024 * 1024:  # 100MB
                            reply = QMessageBox.question(
                                self, "文件过大", 
                                f"文件大小 {self.format_size(file_size)}，超过100MB限制。是否继续上传？",
                    ...
                    if not cur_dir.startswith('/用户上传'):
                        QMessageBox.warning(self, "上传", "用户态上传路径必须在 /用户上传 下")
                        return
                    remote_path = posixpath.join(cur_dir, os.path.basename(path))
                    self.status_label.setText("正在上传文件...")
                    resp = self.api_client.user_upload_local_file(path, remote_path)
                    self._show_result_msg(resp, "上传文件")
                    self.refresh_user_files()
                return
            if action == act_upload_text:
